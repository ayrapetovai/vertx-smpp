...
== Client Features
- Bind to MC as `Transmitter`, `Receiver`, `Tranceiver` by calling bind method and pass host and port to it.
    * User can send `SystemId`, `Password`, `SystemType`, `Address` in bind_request.
    * Also, library sends interface version in bind_request.
- User can set timeouts and callbacks.
- User can send requests `submit_sm`, `deliver_sm`, `query_sm`, `data_sm`.
